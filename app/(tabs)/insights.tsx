...
import { useInsightsAnalytics } from '../../hooks/api';
...
export default function InsightsScreen() {
  const { user } = useAuth();
  const { data: insights, isLoading, error, refetch } = useInsightsAnalytics();
  ...
  if (error || !insights) {
    return (
      <View className="flex-1 bg-slate-50/50 items-center justify-center px-6">
        ...
        <Text className="text-slate-500 text-center mt-4 mb-6">
          {error ? 'Failed to load insights. Please try again later.' : 'No insights available yet. Create some tabs to see your insights!'}
        </Text>
        {error && (
          <TouchableOpacity
            onPress={() => refetch()}
  ...
  // No longer needed: Calculate max amount for chart scaling
        ...
        <View className="flex-row gap-4 mb-8">
          ...
          <Card className="flex-1 p-4 items-center justify-center py-6">
            ...
            <Text className="text-xl font-bold text-indigo-600">
              {(insights.averageSettleTime ?? 0).toFixed(1)} Days
            </Text>
          </Card>
        </View>
          ...
          <Card className="p-6">
            {insights.categoryStats.length > 0 ? (
              <>
                ...
                <View className="items-center mb-6 overflow-hidden">
                  <CategoryPieChart
                    data={insights.categoryStats.map(stat => {
                      const category = TAB_CATEGORIES[stat.category as TabCategory] || TAB_CATEGORIES.OTHER;
                      return {
        ...
        <View className="mb-8">
          ...
          <Card className="p-8">
            {insights.timeline.length > 0 ? (
              <SpendingChart data={insights.timeline} />
            ) : (
              <Text className="text-slate-400 text-center py-8">
